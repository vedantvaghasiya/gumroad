...
import logoGSrc from "$assets/images/logo-g.svg";
...
const SubscribeButton = ({ isSubmitting }: { isSubmitting: boolean }) => (
  <button
    type="submit"
    disabled={isSubmitting}
    className="relative z-3 inline-flex h-12 items-center justify-center rounded-sm border border-black bg-pink px-3 text-base text-black no-underline transition-all duration-150 group-hover:-translate-x-2 group-hover:-translate-y-2 lg:h-12 lg:px-6 lg:text-base"
  >
    <div className="absolute inset-0 z-[-1] -translate-x-1 -translate-y-1 rounded-sm border border-black bg-yellow transition-transform duration-150" />
    <div className="absolute inset-0 z-[-2] translate-x-1 translate-y-1 rounded-sm border border-black bg-red transition-transform duration-150" />
    {isSubmitting ? "..." : "\u2192"}
  </button>
);
    ...
    <div className="flex flex-col justify-between gap-16 bg-black px-8 py-16 leading-relaxed text-white lg:flex-row lg:px-[4vw] lg:py-24">
      <div className="flex w-full max-w-3xl flex-col gap-16">
        ...
        <div className="flex items-center gap-2">
          <img src={logoGSrc} alt="Gumroad icon" className="h-6 w-6" />
          <div>&copy; Gumroad, Inc.</div>
        </div>
      </div>
      <div className="flex w-full max-w-3xl flex-col gap-16">
        <div className="flex flex-1 gap-16">
          <div className="flex flex-1 flex-col gap-3">
            ...
            <Link href={Routes.pricing_path()} className="no-underline hover:text-pink">
              Pricing
            </Link>
            <Link href={Routes.features_path()} className="no-underline hover:text-pink">
              Features
            </Link>
            <Link href={Routes.about_path()} className="no-underline hover:text-pink">
              About
            </Link>
            <Link href={Routes.small_bets_path()} className="no-underline hover:text-pink">
              Small Bets
            </Link>
          </div>
          <div className="flex flex-1 flex-col gap-3">
            ...
            <Link href={Routes.terms_path()} className="no-underline hover:text-pink">
              Terms of Service
            </Link>
            <Link href={Routes.privacy_path()} className="no-underline hover:text-pink">
              Privacy Policy
            </Link>
          </div>
        </div>
