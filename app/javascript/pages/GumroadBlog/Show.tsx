...
const BackToBlog = ({ className }: { className?: string }) => (
  <div className={className}>
    <Link href="/blog" className="mt-4 flex items-center font-medium text-pink-600 hover:text-pink-800">
...
function GumroadBlogShow() {
  const { external_id, subject, published_at, message, call_to_action } = cast<Props>(usePage().props);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white font-['ABC_Favorit'] text-base leading-relaxed font-normal tracking-tight text-black">
      <HomeNav />
...
GumroadBlogShow.layout = (page: React.ReactNode) => <LoggedInUserLayout>{page}</LoggedInUserLayout>;

export default GumroadBlogShow;
